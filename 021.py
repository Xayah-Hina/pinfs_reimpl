...
def create_aabb(args):
    ...
    return aabb, train_data, bkg_color, near, far, t_info
...
def pinf_train():
    ...
    aabb, _1, bkg_color, near, far, t_info = create_aabb(args)
    ...
    for i in trange(start, args.N_iter + 1):
        model_fading_update(model, prop_model, None, global_step, None)
        tempo_fading = fade_in_weight(global_step, 0, args.tempo_fading)

        video_i, frame_i = divmod(np.random.randint(dataset_native.num_frames * dataset_native.num_videos), dataset_native.num_frames)
        target = dataset_native.images[frame_i][video_i]
        H, W = target.shape[:2]
        focal = dataset_native.focals[video_i].item()
        K = np.array([
            [focal, 0, 0.5 * W],
            [0, focal, 0.5 * H],
            [0, 0, 1]
        ], dtype=np.float32)
        pose = dataset_native.poses[video_i]
        time_locate = t_info[-1] * frame_i

        if (args.vggW > 0.0) and (i % 4 == 0):  # get a cropped img (dw,dw) to train vgg
            coords_crop, dw = vgg_sample(args.vgg_strides, args.N_rand, target, bkg_color, steps=i)
